    ...
    for lot in lots:
        ...
        if should_alert and valuation:
            ...
            alerts.append({
                ...
                "confidence_label": _confidence_label(valuation.confidence),
                "live_cap": cap,
                "margin_pct": ((valuation.fair_value - unit_price) / valuation.fair_value * 100) if valuation.fair_value and valuation.fair_value > 0 else 0,
                "lot_key": key,
            })
...
async def send_discord_alerts(alerts: list[dict]):
    ...
    for alert in alerts[:10]:  # cap at 10 alerts per scan
        ...
        if DISCORD_ALERT_WEBHOOK:
            ...
            if channel is None:
                log.error("Configured CHANNEL_ID is not visible to the bot")
            else:
                try:
                    await channel.send(embed=embed)
                except discord.Forbidden:
                    log.error("Bot lacks permission to send to configured CHANNEL_ID")
                except Exception as e:
                    log.error("Failed to send to configured CHANNEL_ID: %s", e)
        else:
...
async def cmd_price(interaction: discord.Interaction, item_name: str):
    """Get price valuation for an artifact using the price model."""
    ...
    for qlt in sorted(by_tier.keys()):
        ...
        result = await evaluate_lot_with_model(market_db, item_id, actual_name, qlt, bonus)
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"Confidence: {result.confidence}/100 ({_confidence_label(result.confidence)})"
